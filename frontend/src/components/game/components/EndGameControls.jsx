...
const EndGameControls = forwardRef(function EndGameControlsModal(
  ...
  return (
    ...
    >
      <div className="modal-box glass bg-white/60 dark:bg-slate-800/80 shadow-2xl border border-white/20 backdrop-blur-lg">
        <h1 className="text-5xl font-extrabold text-center mb-4 text-gray-900 dark:text-white">
          {outcomeMessage}
        </h1>

        {user && (
          <div className="flex justify-center gap-6 mb-6 text-lg font-semibold text-gray-700 dark:text-gray-300">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 mt-4">
          <button onClick={handleNewGame} className="btn btn-success btn-lg font-bold w-full sm:w-auto">
            🎮 New Game
          </button>
          <button onClick={handleQuit} className="btn btn-error btn-lg font-bold w-full sm:w-auto">
            🚪 Quit & Logout
          </button>
        </div>
      </div>
